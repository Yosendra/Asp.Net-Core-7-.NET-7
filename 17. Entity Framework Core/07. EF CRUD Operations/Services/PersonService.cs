...
public class PersonService : IPersonService
{
    ...
    // Inject PersonDbContext, also CountryService object
    ...
    public PersonResponse AddPerson(PersonAddRequest? requestModel)
    {
        ...
        // Access Person table through '_db'
        _db.Persons.Add(person);

        // Remember this to actual changes in table data
        _db.SaveChanges();
    ...
    {
        // This is error when we already use EFCore, we cannot invoke our custom private function inside LINQ of EFCore
        //return _db.Persons.Select(p => ConvertPersonToPersonResponse(p)).ToList();

        return _db.Persons.ToList(). // Eager loading
    ...
    public PersonResponse UpdatePerson(PersonUpdateRequest? requestModel)
    {
        ...
        // Run this command whenever there are some changes in db or entity state

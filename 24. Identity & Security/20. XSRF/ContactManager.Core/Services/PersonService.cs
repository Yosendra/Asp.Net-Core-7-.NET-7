...
public class PersonService : IPersonService
{
    ...
    public async Task<List<PersonResponse>> GetFilteredPersons(string searchBy, string? keyword)
    {
        ...
        using (Operation.Time("Time for GetFilteredPerson() in Database"))
        {
            ...
            {
                nameof(PersonResponse.Name)         => await _personRepository.GetFilteredPerson(p => p.Name.Contains(keyword)),
                nameof(PersonResponse.Email)        => await _personRepository.GetFilteredPerson(p => p.Email.Contains(keyword)),
                nameof(PersonResponse.Gender)       => await _personRepository.GetFilteredPerson(p => p.Gender.Contains(keyword)),
                nameof(PersonResponse.CountryId)    => await _personRepository.GetFilteredPerson(p => p.Country.Name.Contains(keyword)),
                nameof(PersonResponse.Address)      => await _personRepository.GetFilteredPerson(p => p.Address.Contains(keyword)),
                nameof(PersonResponse.DateOfBirth)  => await _personRepository.GetFilteredPerson(p => p.DateOfBirth.Value.ToString("dd MMM yyyy").Contains(keyword)),
    ...
    public async Task<List<PersonResponse>> GetSortedPersons(List<PersonResponse> allPersons, string sortBy, SortOrderEnum sortOrder)
    {
        ...
        {
            (nameof(PersonResponse.Name), SortOrderEnum.ASC)  => allPersons.OrderBy(p => p.Name).ToList(),
            (nameof(PersonResponse.Name), SortOrderEnum.DESC) => allPersons.OrderByDescending(p => p.Name).ToList(),

            (nameof(PersonResponse.Email), SortOrderEnum.ASC)  => allPersons.OrderBy(p => p.Email).ToList(),
            (nameof(PersonResponse.Email), SortOrderEnum.DESC) => allPersons.OrderByDescending(p => p.Email).ToList(),

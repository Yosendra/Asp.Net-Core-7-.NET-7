...
public class AccountController : Controller
{
    ...
    [Authorize("NotAuthenticated")]                 // notice this
    ...
    public async Task<IActionResult> Login(LoginDto loginDto, string? returnUrl)
    {
        ...
        if (result.Succeeded)
        {
            ...
            if (user != null)
            {
                bool isAdmin = await _userManager.IsInRoleAsync(user, UserTypeOptions.Admin.ToString());
                if (isAdmin)                                                                                // check if the user is admin
                    return RedirectToAction("Index", "Home", new { Area = "Admin" });                       // redirect to Admin home page, notice the route argument is a must
            }
    ...
    [Authorize]                                     // notice this, accessible only when user has logged-in

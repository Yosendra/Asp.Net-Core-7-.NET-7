...
public class PersonServiceTest
{
    ...
    public async Task GetFilteredPersons_SearchByName_ListNotEmpty_ReturnMatchesCollection()
    {
        ...
        // Assert
        //foreach (var addedPerson in addedPersonList)
        //    if (addedPerson.Name!.Contains(keyword, StringComparison.OrdinalIgnoreCase))
        //        Assert.Contains(addedPerson, actualPersonSearchList);
    ...
    public async Task GetSortedPersons_SortByNameDescendingOrder_ListNotEmpty_ReturnSortedCollection()
    {
        ...
        /* Assert with Fluent Assertion
        */
        //actualSortedPersonList.Should().BeEquivalentTo(expectedSortedPersonList);
        actualSortedPersonList.Should().BeInDescendingOrder(p => p.Name);                          // more stright way
    }
    #endregion

...
//[TypeFilter(typeof(ResponseHeaderActionFilter),                                               // Before, controller-level
//    Arguments = new object[] { "Key-From-Controller", "Value-From-Controller" },
//    Order = 2)]

[TypeFilter(typeof(ResponseHeaderActionFilter),
    Arguments = new object[] { "Key-From-Controller", "Value-From-Controller", 3 }, Order = 3)]            // Notice the third parameter we pass. that is for "Order"
public class PersonController : Controller
{
    ...
    //[TypeFilter(typeof(PersonListActionFilter))]                                          // Before, action-level

    [TypeFilter(typeof(PersonListActionFilter), Order = 4)]                                 // Here, we use do not implements "IOrderedFilter" in PersonListActionFilter, just use the provided order argument of TypeFilter attribute

    //[TypeFilter(typeof(ResponseHeaderActionFilter),                                                  // Before, action-level
    //    Arguments = new object[] { "Key-From-Action", "Value-From-Action" },
    //    Order = 1)]    

    [TypeFilter(typeof(ResponseHeaderActionFilter),                                   
        Arguments = new object[] { "Key-From-Action", "Value-From-Action", 1 }, Order = 1)]            // Notice the third parameter we pass. that is for "Order"
